...
async getPostByID(key, value) {
  const [rows] = await pool.promise().query(`SELECT  u.login, b.* FROM users u 
  INNER JOIN blogs b ON b.Creator_Id = u.id_User 
  WHERE ${key}='${value}'`);
  return rows;
}
